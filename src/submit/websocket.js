...
const HOST_RAW = pref.getHostName(true);
const TIMEOUT = 45000; // 45 sec
const URL = `wss://pubsub.${HOST_RAW}/ws/`;
// let progressHandler: { report: (arg0: { message: string; increment?: number; }) => void; } | undefined = undefined;
let resultSocket = {
    client: new wsClient(),
    conn: undefined,
};
...
const parseData = (data, result = false) => {
    if (
        !result &&
        (!statusSocket.submissionId || data.d[1] != statusSocket.submissionId)
    )
        return;
    // console.log(data);
...
const isWating = (verdictString) => {
    return (
        !verdictString ||
        verdictString == "null" ||
        verdictString == "TESTING" ||
        verdictString == "SUBMITTED"
    );
};
