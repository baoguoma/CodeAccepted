...
const URL = `${HOST}/problemset/status?my=on`;
 ...
 * @param {String} url - submission status url (optional) default = ${HOST}/problemset/status?my=on
 */
const getUpdate = async (subId, url = undefined) => {
    if (!url) url = URL;
    let accepted = false;
    ...
    while (!accepted) {
        ...
        if (data.waiting == false) {
            ...
            //     ${data.memory}`,
            // });
            vscode.window.showInformationMessage(`${data.statusText}, Time: ${data.time}, Memory: ${data.memory}`);
        ...
        if (data.waiting == true) {
            ...
            // });
            vscode.window.showInformationMessage(`Result: ${data.statusText}`);
        }
    }
};
...
const getStatus = (res, submissionId) => {
    ...
    $("tbody")
        ...
        .each((i, row) => {
            ...
            if (id == submissionId) {
                data.waiting =
                    $(row).find(".status-verdict-cell").attr("waiting") ==
                        "true"
                        ? true
                        : false;
                data.statusText = $(row)
                    .find(".status-verdict-cell")
                    .text()
                    .trim();
                data.time = $(row).find(".time-consumed-cell").text().trim();
                data.memory = $(row)
                    .find(".memory-consumed-cell")
                    .text()
                    .trim();
                //console.log(data);
                return data;
